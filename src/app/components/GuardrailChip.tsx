...
export function GuardrailChip({ guardrailResult }: GuardrailChipProps) {
  ...
  return (
    <div className="rounded-sm border border-[#F6A878]/40 bg-[#2A0D08]/80 px-3 py-2 text-xs text-[#FFE5D2] shadow-inner">
      <div className="flex items-center justify-between gap-3">
        <span className="font-semibold uppercase tracking-[0.12em] text-[#FFC79E]">
          {STATUS_LABEL[status] ?? "Guardrail"}
        </span>
        {category ? (
          <span className="rounded-sm bg-[#F6A878]/20 px-2 py-0.5 text-[11px] font-medium uppercase tracking-[0.08em] text-[#FCD2A8]">
